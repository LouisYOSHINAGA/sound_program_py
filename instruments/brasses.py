...
def trombone(note_no: int, velocity: int, gate: float, duration: float, sr: int =44100) -> np.ndarray:
    freq: float = calc_freq(note_no)
    n_partial: int = int(np.clip(3600/freq, 5, 30))
    ...
    vca1_delays: np.ndarray =  calc_delayar(vco_offsets+vco_depths, p1=1800, p2=0, p3=1800/12, p4=0.03, is_delay=True)
    vca1_as: np.ndarray =  calc_delayar(vco_offsets, p1=1800, p2=0.02, p3=1800/12, p4=0.02, is_sub=True)
    vca1_ss: np.ndarray = np.repeat(1, n_partial)
    vca1_rs: np.ndarray =  calc_delayar(vco_offsets, p1=1800, p2=0.2, p3=1800/12, p4=0.1, is_sub=True)
    vca1_gates: np.ndarray = np.repeat(gate, n_partial)
    vca1_durs: np.ndarray = np.repeat(duration, n_partial)
    vca1_offsets: np.ndarray = np.repeat(0, n_partial)
    vca1_depths: np.ndarray = np.repeat(1, n_partial)
    ...
    y: np.ndarray = biquad_filter(data=z, filter_type="bandpass", fc=900, Q=1/np.sqrt(2), sr=sr)
    y = biquad_filter(data=y, filter_type="lowpass", fc=1800, Q=1/np.sqrt(2), sr=sr)
    return (velocity / 127) / np.max(np.abs(y)) * y