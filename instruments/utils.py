...
def noise(n_sample: int, seed: int =0) -> np.ndarray:  # TODO: delete
...
def calc_delayar(offsets: np.ndarray|float, p1: float, p2: float, p3: float, p4: float, 
                 is_sub: bool =False, is_delay: bool =False) -> np.ndarray:
    delayar: np.ndarray = (1 - 1 / (1 + np.exp(-(offsets - p1) / p3))) * p4 + p2 if is_sub \
                          else 1 / (1 + np.exp(-(offsets - p1) / p3)) * p4 + p2
